...
// }}}
/**
 * class RenderNodeDropBox
 *  Renders a Node using a dropbox to display the claims
 * @package JustReason_Engine
 * @subpackage Display
 */
class RenderNodeDropBox extends RenderNodeText {
   // {{{ __CONSTRUCT( & $node )
   /**
    * Constructor for RenderNodeDropBox
    *
    * @access public
    *
    * @param Node
   ...
   public function getClaims(){
      ...
      if(method_exists( $this->node, "numClaims") ){
         if( $this->node->getClaimId() !== NOT_SET ){
            ...
            $html .= "<select name='SELECT_".$this->node->getId()."'  class='claimvalue'> ";
            for( $xx = 0; $xx < $this->node->numClaims(); $xx++  ){
     
         
               if( $this->node->getClaimId() == $xx ){
                  $html .= "<option value='".$xx."' selected='true'>".
